...
def detect(save_img: bool=False):
    (
        source,
        weights,
        weights_second,
    ...
    if len(weights_second) > 0:
        ...
        model_second = attempt_load(
            weights_second, map_location=device
        )  # load FP32 model
    ...
    if device.type != "cpu":
        ...
        model_second(
            ...
            .to(device)
            .type_as(next(model_second.parameters()))
        )  # run once
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                cropped_dets[:, 2] = (
                    (cropped_dets[:, 2] - cropped_dets[:, 0]) * 0.7
                ) + cropped_dets[:, 0]
                ...
                for i, det_second in enumerate(cropped_dets[:, :4]):
                    ...
                    # Crop image
                    cropped_im0 = im0[
                        det_second[1] : det_second[3], det_second[0] : det_second[2]
                    ]
                    ...
                    # Convert
                    img_second = img_second[:, :, ::-1].transpose(
                        2, 0, 1
                    ...
                    img_second = (
                        img_second.half() if half else img.float()
                    )  # uint8 to fp16/32
                    ...
                    pred_second = non_max_suppression(
                        pred_second,
                        opt.conf_thres,
                        opt.iou_thres,
                        classes=opt.classes,
                ...
                for phone_det in pred_second:
                    if len(phone_det):
                        ...
                        phone_det[:, :4] = scale_coords(
                            img_second.shape[2:], phone_det[:, :4], cropped_im0.shape
                        ).round()
                        # output = det[:, :4].tolist()[0]
                        # output = [int(x) for x in output]
                        ...
                        draw_boxes_second(
                            ...
                            [
                                [
                                    det_second[0] + phone_det[0],
                                    det_second[1] + phone_det[1],
                                    (phone_det[2] - phone_det[0])
                                    + det_second[0]
                                    + phone_det[0],
                                    (phone_det[3] - phone_det[1])
                                    + det_second[1]
                                    + phone_det[1],
                                ]
                            ],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--weights_second", nargs="+", type=str, default="", help="model.pt path(s)"
    )
